...
refractory_periods = [0.001, 0.003, 0.005]
cvals = [0.5]

for tr in refractory_periods:
    for c in cvals:

        params = ParamsLIF(sigma = sigma, mu = mu, tau = tau, tr = tr)
        lif = LIF(params, t = t)
        lif.x = 0
        n = params.n

        print("Running simulations for c = %f"%c)
        outfile = './data/output/param_w_N_%d_nsims_%d_c_%f_tr_%f_deltaT_counterfactual_simulations.npz'%(N, nsims, c, tr)
        params.c = c
        lif.setup(params)
        lif.x = 0
